...
- [[2. Pointers, Arrays And References#Arrays|Arrays]]: explains the raw buffers passed to `send()`, `recv()`, and related APIs.
- [[12. Memory Management#C++ Program Memory|Program memory]]: helps distinguish stack buffers, heap buffers, and resource lifetime.
- [[8. Compiler#include and Translation Units|Headers and translation units]]: connects required socket headers with declarations provided by the system.
- [[24. Threads And Lambdas#Why Parallel Programming|Parallel programming]]: motivates the chat example where input and network I/O must proceed concurrently.
- [[26. Inter-thread Communication#Producer Consumer Problem|Producer-consumer communication]]: is useful when socket I/O is split across worker threads.
- [[20. Standard Library#I/O Streams|I/O streams]]: contrasts C++ stream I/O with POSIX socket descriptors.
