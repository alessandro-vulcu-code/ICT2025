...
- The outline must be clickable in Obsidian: every bullet must link to a real heading in the same file using heading wiki-links, for example `[[#Declarations|Declarations]]`.
...
- Make sure linked heading text stays exactly aligned with the real heading if you edit section titles.
