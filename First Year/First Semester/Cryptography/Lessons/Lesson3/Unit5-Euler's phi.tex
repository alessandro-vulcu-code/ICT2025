...
\begin{definition}[Euler's Totient Function, $\phi$]
    Let $m \in \mathbb{N}$ such that $m \ge 1$. The \textbf{Euler's totient function}, denoted by $\phi(m)$, is the number of elements in the multiplicative group of integers modulo $m$, which is $\left(\mathbb{Z}/m\mathbb{Z}\right)^*$.
...
\begin{definition}[Remainder Map]
    Let $m, n \in \mathbb{N}$ be integers. The \textbf{remainder map} $r$ is a function:
...
\begin{theorem}[Bijectivity of the Remainder Map with Coprime Factors]
    Let $m, n \in \mathbb{N}$ be integers such that $m, n \ge 1$, and assume $m$ and $n$ are \textbf{pairwise coprime} (i.e., $\gcd(m, n) = 1$).
    
    The remainder map $r$:
    $$r: \mathbb{Z}/(mn)\mathbb{Z} \longrightarrow (\mathbb{Z}/m\mathbb{Z}) \times (\mathbb{Z}/n\mathbb{Z})$$
    is a \textbf{bijection} (one-to-one and onto).
\end{theorem}

\begin{proof}
    The proof establishes bijectivity by demonstrating both \textbf{Surjectivity} (the map is onto) and \textbf{Injectivity} (the map is one-to-one).
    ...
    Since we are given that $\gcd(m, n) = 1$, the \textbf{Chinese Remainder Theorem} guarantees that a solution $x$ always exists for any pair of residues $(a, b)$.
    
    Therefore, $\forall$ element in the codomain, there is a corresponding pre-image in the domain, proving that the map $r$ is \textbf{surjective}.
    
    \subsection*{Injectivity (One-to-One)}
    We can establish injectivity by counting the elements (since the sets are finite):
    
    \begin{enumerate}
        \item \textbf{Count the Domain:} The number of elements in the domain is the modulus:
        $$|\mathbb{Z}/(mn)\mathbb{Z}| = mn$$
        
        \item \textbf{Count the Codomain:} The number of elements in the codomain (a Cartesian product) is the product of the sizes of the factors:
        $$|(\mathbb{Z}/m\mathbb{Z}) \times (\mathbb{Z}/n\mathbb{Z})| = |\mathbb{Z}/m\mathbb{Z}| \cdot |\mathbb{Z}/n\mathbb{Z}| = m \cdot n = mn$$
        
        \item \textbf{Conclusion:} Since the domain and codomain are finite sets and have the \textbf{same number of elements} ($mn = mn$), and we have already proven the map $r$ is \textbf{surjective}, it must automatically be \textbf{injective} as well.
        
        $$\text{Surjectivity} \land (|\text{Domain}| = |\text{Codomain}|) \implies \text{Injectivity}$$
    \end{enumerate}
    
    Since $r$ is both surjective and injective, it is a \textbf{bijection}.
\end{proof}
