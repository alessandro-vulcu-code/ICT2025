...
\chapter[Lesson 25-11-2025]{Lesson 25-11-2025}
...
\begin{tikzpicture}[auto, node distance=1.8cm, >=latex']
    % Stili
    \tikzstyle{block} = [draw, rectangle, rounded corners, minimum height=2em, minimum width=2.5em, align=center]
    ...
    % Nodi
    \node [input] (in) {};
    \node [left of=in, node distance=0.8cm] (lbl) {$\pm 1$};
    ...
    \node [below of=noise, node distance=0.2cm] {$w$};
    ...
    % Annotazione EQ
    \node [below of=eq, node distance=0.8cm] {?};
\end{tikzpicture}
\end{center}
\vspace{1em}

\begin{tcolorbox}[colback=obsidianGreen, colframe=white, boxrule=0pt, sharp corners]
...
\begin{tikzpicture}[auto, node distance=2cm, >=latex']
    \tikzstyle{block} = [draw, rectangle, rounded corners, minimum height=2em, minimum width=3em]
    \node [block] (eq) {EQ};
    \node [block, right of=eq] (dec) {dec};
    \node [right of=dec, node distance=2.5cm] (out) {$\hat{x}(0), \hat{x}(1)$};

    \draw [->] (eq) -- (dec);
    \draw [->] (dec) -- (out);
    
    % Feedback loop
    \draw [->] (dec.east) -- ++(0.5,0) -- ++(0,1) -| (eq.north);
\end{tikzpicture}
\end{center}
...
What we do is that we go in the $z$ domain (see \textcolor{ForestGreen}{z transform}); when we have a sequence, such as $h[0], h[1], h[2], \dots$, we'll write it as
\begin{align}
...
\begin{tikzpicture}[auto, node distance=1.5cm, >=latex']
    ...
    \node[draw=orange, thick, fit=(invh) (rx), inner sep=5pt, rounded corners=2pt] {};

    \node [right of=rx, node distance=3.5cm, align=left, font=\small\itshape] (note) {IIR + noise enhancement\\(written in book)};
...
\begin{tikzpicture}[>=latex']
    ...
    % Linea verticale di allineamento
...
\begin{tcolorbox}[colback=obsidianBlue, colframe=white, boxrule=0pt, sharp corners]
...
\begin{tcolorbox}[colback=obsidianPurple, colframe=white, boxrule=0pt, sharp corners]
...
\begin{tcolorbox}[colback=obsidianYellow, colframe=white, boxrule=0pt, sharp corners]
...
\begin{tcolorbox}[colback=obsidianPurple, colframe=obsidianBorderPurple, boxrule=1pt, rounded corners]
...
\begin{tcolorbox}[colback=white, colframe=obsidianBorderPurple, boxrule=1pt, rounded corners]
    ...
    \begin{center}
    $\downarrow$ \textit{read example (continued)}
    \end{center}
\end{tcolorbox}
...
\end{align}
and also such that $\boldsymbol{A}\boldsymbol{A}^\dagger, \boldsymbol{A}^\dagger \boldsymbol{A}$ are both \textcolor{ForestGreen}{Hermitian}.
...
\textcolor{red}{\faExclamationTriangle\ The right inverse and the left inverse are not the same!}\\
\textcolor{red}{\faExclamationTriangle\ This only holds if the matrix is \textbf{full-rank} (which is then the reason why we ask that the $\boldsymbol{H}$ matrix is full-rank).}\\
\textcolor{red}{We'll use this a lot in the course.}
...
\begin{tcolorbox}[colback=obsidianBlue, colframe=white, boxrule=0pt, sharp corners]
    \textcolor{obsidianTextBlue}{\textbf{\faAtom\ $\mathbf{W}^{ZF}$}}
...
\begin{tcolorbox}[colback=obsidianBlue, colframe=white, boxrule=0pt, sharp corners]
    \textcolor{obsidianTextBlue}{\textbf{\faAtom $\bar{\mathbf{D}}$}}
...
\begin{tcolorbox}[colback=teal!5, colframe=teal, boxrule=0.5pt, sharp corners]
...
\begin{tcolorbox}[colback=blue!5, colframe=blue, boxrule=0.5pt, sharp corners]
...
\textit{\textcolor{ForestGreen}{It can be shown}} that thanks to the transmit-receive relationships we defined:
\begin{align}
...
\begin{tcolorbox}[colback=violet!5, colframe=violet, boxrule=0.5pt, sharp corners]
...
\end{align}
which in this case (that of \textit{\textcolor{ForestGreen}{WSS signals/noise}}) yields
\begin{align}
...
\begin{tcolorbox}[colback=red!5, colframe=red, boxrule=0.5pt, sharp corners]
...
\begin{tcolorbox}[colback=orange!5, colframe=orange, boxrule=0.5pt, sharp corners]
...
\begin{tcolorbox}[colback=violet!5, colframe=violet, boxrule=0.5pt, sharp corners]
    ...
    \begin{tcolorbox}[colback=white, colframe=blue, boxrule=0.5pt, sharp corners]
...
The alternative form is obtained using the \textbf{\textcolor{ForestGreen}{Matrix inversion lemma}}.
