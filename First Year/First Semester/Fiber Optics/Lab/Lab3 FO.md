...
5.  **Acquisizione Dati su Analizzatore di Spettro**: Per ogni punto di misura, è stata seguita una procedura di acquisizione standardizzata:
    ...
    * Sono stati impostati due marker, posizionati rispettivamente a $\mathbf{1\ \text{nm}}$ **prima** e $\mathbf{1\ \text{nm}}$ **dopo** la lunghezza d'onda del picco del segnale, per misurare i livelli di potenza del rumore $\text{ASE}$ in prossimità del segnale. Questo è dovuto al fatto che il segnale d'ingresso ha un'influenza sull'ASE e non potendo misurare il valore nel punto esatto, facciamo una stima interpolando i valori misurati prima e dopo.
