...
(b) Let $$
W_{tj}^{(n)} = \mathbb{E}\left[
\sum_{k=0}^{n-1} \mathbf{1}_{\{X_k = j\}}
\,\middle|\, X_0 = i
\right]
$$ be the average number of visits to state $j$ during the first $n$ time slots, given that the chain starts in state $i$. Compute $\lim_{n \to \infty} W_{tj}^{(n)}$ for $j = 0, 1, 2$.
