...
(b) Let $W_{ij}^{(n)} = E \left[ \sum_{k=0}^{n-1} I \{X_k = j\} \mid X_0 = i \right]$ be the average number of visits to state $j$ during the first $n$ time slots, given that the chain starts in state $i$. Compute $\lim_{n \to \infty} W_{ij}^{(n)}$ for $j = 0, 1, 2$.
...
(b) Compute $P[X_2(1) = 1|X_2(1) + X_2(2) = 3]$ and $P[X_2(1) + X_2(2) = 3|X_2(1) = 1]$
