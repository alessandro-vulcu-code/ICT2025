...
2. What is the guaranteed size order for fundamental types in C++?
...
3. Why do I get strange results when printing an `uint_fast8_t`?
...
4. Explain how the switch-case statement works, providing an example.
```cpp
switch(variable){
    case 1:
        // case variable == 1
...
5. After the operations: `uint_fast8_t a = 21; uint_fast8_t b = 11; uint_fast8_t c = a ^ b;` Which is the value of `c`?
* **Answer:** 30
...
7. What is a reference? Provide an example.
...
8. We want to pass a `string s`, an `integer i` and a `double d` as input parameters to a function (they cannot be modified by the function, and it should be efficient). How do we pass them?
...
9. What is the technical difference between a `struct` and a `class`?
...
11. Why is it sometimes better to use `()` instead of `{}` when calling a constructor?
...
12. If no copy or move constructors are declared, what does the compiler do, and how can you explicitly prevent their creation?
...
13. What happens to the default move constructor if you manually declare only a copy constructor for a class?
* "if one among a copy/move/destructor operation is declared, no copy/move/destructor operations are default-generated"


14. When a derived class `B` is instantiated, in what order are the constructors called, and how can the base class `A` be initialized?
...
15. Why is it critical to declare the destructor of a base class as `virtual`?
...
16. What is the purpose of `public virtual` inheritance?
...
17. What is **Object Slicing**, and under what circumstances does it occur during object assignment or passing?
...
18. Explain how the `this` pointer can be used to avoid **shadowing** and how it enables **concatenated operations** (method chaining).
...
19. Provide an example of how to overload the `<<` operator to allow polymorphic printing for a base class `EthConn` and its derived class `TcpServer`.
```cpp
...
20. Where should the implementation of a template class be defined?
...
21. Each time you write the word `new`, you then need to write the word `delete`. Who is responsible to perform this operation?
* Packet destructor must delete each inserted element, as it owns them now. So, it is the owner of the resource.


22. An object has been created with `new`; where and when is the object going to be destroyed with `delete`?
...
23. What is memory leak?
* See question nr. 22 (It happens when memory allocated on the heap is not deleted).


24. What is a dangling pointer? And double deletion?
...
25. Why can the default copy assignment cause problems (shallow copy)?
...
26. Explain the difference between **Shallow Copy** and **Deep Copy**. Why is a shallow copy problematic when a class contains a pointer member (raw, shared, or weak)?
...
27. Describe the **RAII (Resource Acquisition Is Initialization)** pattern in the context of socket management. How does the class destructor ensure resource safety?
...
28. When should I use `unique_ptr`, `shared_ptr` and `weak_ptr`?
...
29. How do I pass a `unique_ptr` to functions?
...
31. Explain the purpose of `std::weak_ptr`. In which scenarios is it necessary to use it instead of a `std::shared_ptr`?
...
32. What is the effect of `std::move()` on an lvalue, and what happens to the original object's resource ownership after the move?
...
33. Consider the following code snippet: `void func(int&& r); int b = 10;`. Why does `func(b)` result in a compiler error, while `func(std::move(b))` is acceptable?
...
34. When should `static_cast`, `dynamic_cast`, `const_cast`, and `reinterpret_cast` be used?
...
36. What is the difference between `vector`/`deque` and `list`/`forward_list` in terms of memory allocation?
...
39. How do the `begin()` and `end()` methods work with iterators?
...
40. How do the algorithms `all_of`, `any_of`, and `none_of` function?
...
41. How does the `insert` method work in a `std::list`?
...
43. Describe the process state diagram and the possible transitions between states (e.g., Ready, Running, Waiting).
...
44. Who is responsible for switching between processes?
...
45. What is the definition of a Thread?
...
46. What’s going on when multiple threads are running in parallel? What is a race condition?
...
47. What happens if two or more threads access a critical region without any thread safe mechanism?
...
48. Define a critical region (or critical section).
* Critical region: part of the code that represents a race condition, so the shared access to the resources.


49. What is a mutex and what is its primary purpose in multithreading?
...
50. When handling multiple clients with threads, what is the functional difference between using `std::thread::detach()` and `std::thread::join()`?
...
51. What are condition variables and how are they used to synchronize threads?
...
52. How do you pass a variable by reference to a `std::thread`?
...
44. Considering the use of Lambdas in Threads, which is the value of `a` after joining `thr` in each of these cases? (Case A: ref, Case B: val arg, Case C: val mutable)
**Case A:**
```cpp
    int main() {
        int a = 0;
...
54. How do Lambda capture clauses (`[]`) work?
...
55. What is the effect of the `mutable` keyword in a lambda?
...
56. What are atomic variables? How can I use them for exit flags?
...
* **Key Methods:** `load()`, `store(val)`, `exchange(val)`.
...
57. According to the "code reviewer" mindset, what are the key points to check to ensure a class is "exam-ready" regarding const correctness and thread safety?
