...
\noindent\colorbox{yellow!25}{
    \parbox{\dimexpr\linewidth-2\fboxsep}{
        ...
        \small
        \textbf{Q:} Is there a constraint on the length of the Cyclic Prefix? \\
        \textbf{A:} Yes. The length of the Cyclic Prefix ($L$) must be greater than or equal to the length of the channel impulse response. This ensures that all Inter-Block Interference (IBI) falls within the cyclic prefix, which is then discarded at the receiver, leaving the data block "clean".
    }
}
...
\noindent\colorbox{yellow!25}{
    \parbox{\dimexpr\linewidth-2\fboxsep}{
        ...
        \small
        \textbf{Q:} Confusion regarding $n$ vs $k$ notation in the final proof. \\
        \textbf{A:} Some ambiguity was noted in the use of indices. In the final formula, $n$ is used as the output subcarrier index ($\tilde{y}(n)$). The formula proves that the output at index $n$ depends \textit{only} on the input at the same index $n$ ($x[n]$), scaled by the channel $h[n]$. This confirms the absence of Inter-Carrier Interference (ICI).
    }
}
